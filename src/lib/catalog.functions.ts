...
const pilotImportSchema = z.object({
  ...
  skill: z.union([z.number(), z.string()]).optional(),
  ...
  .handler(async ({ data, context }) => {
    ...
    const normalized = data.items.map((p) => {
      ...
      return {
        ...
        ship_xws,
        skill: typeof p.skill === "string" ? parseInt(p.skill, 10) || 0 : p.skill ?? 0,
        points: typeof p.points === "string" ? parseInt(p.points, 10) || 0 : p.points ?? 0,
